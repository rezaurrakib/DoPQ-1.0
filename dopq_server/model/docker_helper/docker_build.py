...
def unzip_docker_files(filename, target_dir):
    ...
    """
    print("Filename in unzip_docker_files: ", filename)
    print("target directory in unzip_docker_files: ", target_dir)
...
def build_image(filename, dockerfile, unzip_dir="", tag=None, logger = LOG):
    """
    Written by Reza, a replica of build_image() below without unzipping() call
    :param filename:
    :param logger:
    :return:
    """
    ...
    if tag is None:
        tag = filename.lower()
        LOG.warning("Please explicitly provide a tag for the building process. If None is given then the "
